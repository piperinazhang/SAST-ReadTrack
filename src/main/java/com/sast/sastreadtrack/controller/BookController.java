...
public class BookController {
    ...
    public ResponseEntity<Map<String, Object>> deleteBook(@PathVariable Long bookId,HttpSession session) {
        Map<String, Object> result = new HashMap<>();
        try {
            Long userId = ((User) session.getAttribute("user")).getId();
     ...
     */
    @GetMapping("/list")
    public ResponseEntity<List<Book>> getBooks(@RequestParam(required = false) String status,@RequestParam(defaultValue = "1") int pageNum,
     ...
     */
    @GetMapping("/search")
    public ResponseEntity<List<Book>> searchBooks(@RequestParam String keyword,HttpSession session) {
        Map<String, Object> result = new HashMap<>();
        try {
            Long userId = ((User) session.getAttribute("user")).getId();
     ...
     */
    @GetMapping("/stats")
    public ResponseEntity<Map<String, Object>> getReadingStats(HttpSession session) {
        Map<String, Object> result = new HashMap<>();
        try {
            Long userId = ((User) session.getAttribute("user")).getId();
